...
def telegram_webhook(request):
    ...
    try:
        ...
        if 'message' in update_data:
            ...
            # Handle other messages with AI response
            elif text not in ['/start', '/check', '/week']:
                try:
                    import requests
                    
                    logger.info(f"Getting AI response for: '{text}' from {user_name}")
                    ...
                    response = requests.post(url, json=payload)
                    logger.info(f"AI message response: {response.status_code} - {response.text}")
